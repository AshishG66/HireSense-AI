...
async function runTests() {
  console.log('=== STARTING PRODUCTION SMOKE TEST WORKFLOW ===\n');

  const timestamp = Date.now();
  const candidateEmail = `candidate_prod_${timestamp}@test.com`;
  const recruiterEmail = `recruiter_prod_${timestamp}@test.com`;
  ...
  // Helper axios client pointing to production
  const client = axios.create({ baseURL: BACKEND_URL });

  // 1. Candidate Registration
  try {
    const res = await client.post('/auth/register', {
      name: 'Candidate Prod Smoke',
  ...
  const adminEmail = `admin_prod_${timestamp}@test.com`;
  let adminToken = '';
  try {
    const res = await client.post('/auth/register', {
      name: 'Admin Prod Smoke',
      email: adminEmail,
      password,
      role: 'CANDIDATE',
    });
  ...
  if (recruiterToken) {
    try {
      const res = await client.post(
        '/jobs',
        {
          title: 'Full Stack Engineer (Prod Smoke)',
  ...
  if (recruiterToken && codingQuestionId) {
    try {
      const res = await client.post(
        '/assessments/tests',
        {
          title: 'Algorithmic Test Challenge (Prod Smoke)',
  ...
  console.log('\n=== PRODUCTION SMOKE TEST WORKFLOW COMPLETED ===');
